...
class CTaxRegister {
  public:
      CTaxRegister() {
          citizens.reserve(10000);
      }
      ~CTaxRegister() = default;
  
      bool birth(const std::string &name, const std::string &addr, const std::string &account) {

          if (findByName(name, addr) || findByAccount(account)) return false;

          if (citizens.size() == citizens.capacity()) {
              size_t newCapacity = citizens.capacity() * 2;
              if (newCapacity == 0) newCapacity = 1; 
              citizens.reserve(newCapacity);
              rebuildRegistries(); 
          }

          citizens.emplace_back(name, addr, account);
          Citizen* newCitizen = &citizens.back();

          nameRegistry.insert(
              std::lower_bound(nameRegistry.begin(), nameRegistry.end(), newCitizen,
                  [](const Citizen* a, const Citizen* b) { return *a < *b; }), newCitizen);

          accountRegistry.insert(
              std::lower_bound(accountRegistry.begin(), accountRegistry.end(), newCitizen,
                  [](const Citizen* a, const Citizen* b) { return a->account < b->account; }), newCitizen);

          return true;
      }

      bool death(const std::string &name, const std::string &addr) {
          auto it = findByName(name, addr);
          if (!it) return false;
  
          nameRegistry.erase(std::find(nameRegistry.begin(), nameRegistry.end(), it));
          accountRegistry.erase(std::find(accountRegistry.begin(), accountRegistry.end(), it));
      ...
      CIterator listByName() const {
          return CIterator(nameRegistry);
      }
  
      private:
        std::vector<Citizen> citizens;
        std::vector<Citizen*> nameRegistry; 
        std::vector<Citizen*> accountRegistry; 
    
        void rebuildRegistries() {
            nameRegistry.clear();
            accountRegistry.clear();
            for (Citizen& citizen : citizens) {
                nameRegistry.push_back(&citizen);
                accountRegistry.push_back(&citizen);
            }
            std::sort(nameRegistry.begin(), nameRegistry.end(), 
                [](const Citizen* a, const Citizen* b) { return *a < *b; });
            std::sort(accountRegistry.begin(), accountRegistry.end(), 
                [](const Citizen* a, const Citizen* b) { return a->account < b->account; });
        }

        Citizen* findByName(const std::string &name, const std::string &addr) {
            auto it = std::lower_bound(nameRegistry.begin(), nameRegistry.end(), Citizen{name, addr, ""},
                [](const Citizen* a, const Citizen& b) { return *a < b; });
            return (it != nameRegistry.end() && (*it)->name == name && (*it)->addr == addr) ? *it : nullptr;
        }
    
        Citizen* findByAccount(const std::string &account) {
            auto it = std::lower_bound(accountRegistry.begin(), accountRegistry.end(), Citizen{"", "", account},
                [](const Citizen* a, const Citizen& b) { return a->account < b.account; });
            return (it != accountRegistry.end() && (*it)->account == account) ? *it : nullptr;
        }
};
