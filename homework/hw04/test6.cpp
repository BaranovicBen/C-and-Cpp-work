...
std::ostream& operator<<(std::ostream& os, const CMail& x) {
    ...
    if (x.subject()) os << ", subject: " << *x.subject();
    return os;
}
...
class CMailLog {
    std::multimap<CTimeStamp, CMail> mailMap;
    std::map<std::string, std::string> fromMap;
    std::map<std::string, std::optional<std::string>> subjectMap;
...
public:
    int parseLog(std::istream& in) {
        ...
        while (std::getline(in, line)) {
            ...
            std::string content = trim(message);

            CTimeStamp ts(year, convertMonth(monthStr), day, hour, minute, second);

            if (content.rfind("from=", 0) == 0) {
                fromMap[mailID] = content.substr(5);
            } else if (content.rfind("subject=", 0) == 0) {
                subjectMap[mailID] = content.substr(8);
            } else if (content.rfind("to=", 0) == 0) {
                std::string recipient = content.substr(3);
                auto fromIt = fromMap.find(mailID);
                if (fromIt != fromMap.end()) {
                    CMail mail(ts, fromIt->second, recipient,
                               subjectMap.count(mailID) ? subjectMap[mailID] : std::nullopt);
                    mailMap.emplace(ts, std::move(mail));
                    ++deliveredCount;
                }
            }
        }
    ...
    std::list<CMail> listMail(const CTimeStamp& from, const CTimeStamp& to) const {
        ...
        auto it = mailMap.lower_bound(from);
        while (it != mailMap.end() && it->first.compare(to) <= 0) {
