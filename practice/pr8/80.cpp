...
struct CPublicTransport {
    ...
    std::map<std::string, std::vector<std::pair<int,std::string>>> conAtStop;

    CPublicTransport& addLine(unsigned cost, const std::vector<std::string>& stops) {
        int lineID = int(lines.size());
        lines.push_back({cost, stops});

        for (size_t i = 0; i + 1 < stops.size(); i++) {
            const std::string& u = stops[i];
            const std::string& v = stops[i + 1];
            conAtStop[u].emplace_back(lineID, v);
            conAtStop[v].emplace_back(lineID, u);
        }
        return *this;
    }
    ...
    unsigned findCheapest(const std::string& from,
                          const std::string& to) const
    {
        if (conAtStop.find(from) == conAtStop.end() || conAtStop.find(to)   == conAtStop.end())
        {
            throw std::logic_error("Unknown stop");
        }

        using State = std::tuple<unsigned, std::string, int>;
        auto cmp = std::greater<State>();
        std::priority_queue<State, std::vector<State>, decltype(cmp)> pq(cmp);

        std::map<std::pair<std::string,int>, unsigned> dist;

        dist[{from, -1}] = 0;
        pq.emplace(0, from, -1);

        while (!pq.empty()) {
            auto [cost, u, currLine] = pq.top();
            pq.pop();

            if (cost > dist.at({u, currLine})) continue;

            if (u == to) {
                return cost;
            }

            for (auto& [lineID, v] : conAtStop.at(u)) {
                unsigned nextCost = cost;
                if (lineID != currLine) {
                    nextCost += lines[lineID].cost;
                }
                auto key = std::make_pair(v, lineID);
                if (!dist.count(key) || nextCost < dist[key]) {
                    dist[key] = nextCost;
                    pq.emplace(nextCost, v, lineID);
                }
            }
        }
